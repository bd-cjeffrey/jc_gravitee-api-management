...
describe('GroupsPage', () => {
    ...
    describe('list', () => {
        ...
        it('shows an error message when the query fails', () => {
            mockUseGroupsPaged.mockReturnValue({ data: undefined, isLoading: false, isError: true } as ReturnType<
                typeof useGroupsPaged
            >);
            renderPage();
    ...
    describe('create flow', () => {
        it('creates the group, then follows up with a role update, and shows a success toast', async () => {
            ...
            await waitFor(() => {
                expect(createMutateAsync).toHaveBeenCalledWith({ name: 'My Group', lock_api_role: true, event_rules: [] });
            });
            await waitFor(() => {
                expect(updateMutateAsync).toHaveBeenCalledWith({
                    groupId: 'new-group-id',
                    data: expect.objectContaining({ roles: { API: 'USER', APPLICATION: 'USER' } }),
                });
            });
