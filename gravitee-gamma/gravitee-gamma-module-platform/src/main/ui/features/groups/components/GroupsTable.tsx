...
function isDefaultForNewApis(group: Group): boolean {
    return (group.event_rules ?? []).some(rule => rule.event === 'API_CREATE');
}
    ...
    const columns: DataTableProps<Group>['columns'] = [
        {
            ...
            cell: ({ row }: ColCell<Group>) => (
                <div className="flex items-center gap-2">
                    ...
                    {isDefaultForNewApis(row.original) && (
                        <Badge variant="outline" className="text-xs font-normal">
                            Default for new APIs
                        </Badge>
                    )}
                </div>
            ),
