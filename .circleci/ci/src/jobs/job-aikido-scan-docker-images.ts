 ...
 */
import { Config, Job, commands, reusable, workflow } from '@circleci/circleci-config-sdk';
...
export class AikidoScanDockerImagesJob {
  ...
  public static create(dynamicConfig: Config, environment: CircleCIEnvironment, isProd: boolean, imagesToScan: string[]): Job {
    ...
    // Every image scanned here lives on the registry the build jobs pushed to, so the
    // single login the build jobs use covers them all (Docker Hub on a release, azurecr
    // otherwise).
    ...
    const steps: Command[] = [
      ...
      new commands.Run({
        ...
        command: [
          ...
          `while read -r image; do docker pull "$image"; done < ${IMAGE_LIST_FILE}`,
        ].join('\n'),
      }),
  ...
  public static workflowJobs(
    ...
    const components = [
      { label: 'APIM Portal', image: config.components.portal.image, variants: [undefined] },
      { label: 'APIM Console', image: config.components.console.image, variants: [undefined] },
      { label: 'APIM Management API', image: config.components.managementApi.image, variants: ['alpine', 'debian'] },
      { label: 'APIM Gateway', image: config.components.gateway.image, variants: ['alpine', 'debian'] },
    ...
    return [
      new workflow.WorkflowJob(scanJob, {
        ...
        requires: components.map(({ label }) => `Build ${label} docker image${nameSuffix}`),
      }),
    ];
  }
}
